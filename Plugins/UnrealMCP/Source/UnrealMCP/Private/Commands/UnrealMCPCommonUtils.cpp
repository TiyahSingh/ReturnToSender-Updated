...
{
    // Accept full asset paths (e.g. /Game/Procedural/BP_ProceduralTownGenerator)
    ...
    // Common Content Browser folders used by this project and sample projects
    TArray<FString> CandidatePaths;
    CandidatePaths.Add(TEXT("/Game/Blueprints/") + BlueprintName);
    CandidatePaths.Add(TEXT("/Game/Procedural/") + BlueprintName);
    CandidatePaths.Add(TEXT("/Game/ThirdPerson/Blueprints/") + BlueprintName);
    CandidatePaths.Add(TEXT("/Game/") + BlueprintName);
    ...
    // Fallback: Asset Registry search by asset name
...
UK2Node_CallFunction* FUnrealMCPCommonUtils::CreateFunctionCallNode(UEdGraph* Graph, UFunction* Function, const FVector2D& Position)
{
    ...
    // Array library functions need CallArrayFunction so wildcards type correctly
    if (Function->GetOwnerClass() && Function->GetOwnerClass()->GetName().Contains(TEXT("KismetArrayLibrary")))
    {
        UK2Node_CallArrayFunction* ArrayNode = NewObject<UK2Node_CallArrayFunction>(Graph);
        ArrayNode->SetFromFunction(Function);
        FunctionNode = ArrayNode;
    }
    else
    {
        FunctionNode = NewObject<UK2Node_CallFunction>(Graph);
                                           ...
                                           UEdGraphNode* TargetNode, const FString& TargetPinName)
{
    ...
    {
        // Break existing input links so rewiring works reliably (exec/data pins)
        ...
        if (bConnected)
        {
            ...
            {
                SrcNode->PinConnectionListChanged(SourcePin);
            }
            if (UEdGraphNode* DstNode = TargetPin->GetOwningNode())
            {
                DstNode->PinConnectionListChanged(TargetPin);
                DstNode->ReconstructNode();
            }
        }
        return bConnected;
    }
