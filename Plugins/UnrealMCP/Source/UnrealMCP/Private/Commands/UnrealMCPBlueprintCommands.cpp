...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("set_primitive_collision"))
    {
        return HandleSetPrimitiveCollision(Params);
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Prefer known StaticClass lookups (FindObject often fails if class not loaded yet)
    if (ComponentType.Equals(TEXT("SphereComponent")) || ComponentType.Equals(TEXT("Sphere")) || ComponentType.Equals(TEXT("USphereComponent")))
    {
        ComponentClass = USphereComponent::StaticClass();
    }
    else if (ComponentType.Equals(TEXT("BoxComponent")) || ComponentType.Equals(TEXT("Box")) || ComponentType.Equals(TEXT("UBoxComponent")))
    {
        ComponentClass = UBoxComponent::StaticClass();
    }
    else if (ComponentType.Equals(TEXT("StaticMeshComponent")) || ComponentType.Equals(TEXT("StaticMesh")) || ComponentType.Equals(TEXT("UStaticMeshComponent")))
    {
    ...
    if (Blueprint->SimpleConstructionScript)
    {
        for (USCS_Node* Existing : Blueprint->SimpleConstructionScript->GetAllNodes())
        {
            ...
            {
                return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Component already exists: %s"), *ComponentName));
            }
        }
    }
    ...
    if (NewNode)
    {
        ...
        // Sensible defaults for overlap collection spheres
        if (USphereComponent* Sphere = Cast<USphereComponent>(NewNode->ComponentTemplate))
        {
            double Radius = 80.0;
            if (Params->HasField(TEXT("sphere_radius")))
            {
                Radius = Params->GetNumberField(TEXT("sphere_radius"));
            }
            Sphere->InitSphereRadius(static_cast<float>(Radius));
        ...
        // Parent under default scene root if present
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleGetBlueprintComponents(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!Blueprint->SimpleConstructionScript)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Blueprint has no construction script"));
    }
    ...
    for (USCS_Node* Node : Blueprint->SimpleConstructionScript->GetAllNodes())
    {
        ...
        {
            C->SetBoolField(TEXT("generate_overlap_events"), Prim->GetGenerateOverlapEvents());
            C->SetNumberField(TEXT("collision_enabled"), static_cast<int32>(Prim->GetCollisionEnabled()));
            C->SetStringField(TEXT("collision_profile"), Prim->GetCollisionProfileName().ToString());
        }
        if (UStaticMeshComponent* Mesh = Cast<UStaticMeshComponent>(Node->ComponentTemplate))
        {
            C->SetStringField(TEXT("static_mesh"), Mesh->GetStaticMesh() ? Mesh->GetStaticMesh()->GetPathName() : TEXT("None"));
        }
        if (USphereComponent* Sphere = Cast<USphereComponent>(Node->ComponentTemplate))
        {
            C->SetNumberField(TEXT("sphere_radius"), Sphere->GetUnscaledSphereRadius());
        }
        CompArray.Add(MakeShared<FJsonValueObject>(C));
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetPrimitiveCollision(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    if (!Prim)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Component is not a PrimitiveComponent"));
    }

    FString Profile;
    if (Params->TryGetStringField(TEXT("collision_profile"), Profile) && !Profile.IsEmpty())
    {
        Prim->SetCollisionProfileName(FName(*Profile));
    }

    FString Enabled;
    if (Params->TryGetStringField(TEXT("collision_enabled"), Enabled))
    {
        if (Enabled.Equals(TEXT("NoCollision"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionEnabled(ECollisionEnabled::NoCollision);
        }
        else if (Enabled.Equals(TEXT("QueryOnly"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionEnabled(ECollisionEnabled::QueryOnly);
        }
        else if (Enabled.Equals(TEXT("PhysicsOnly"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionEnabled(ECollisionEnabled::PhysicsOnly);
        }
        else if (Enabled.Equals(TEXT("QueryAndPhysics"), ESearchCase::IgnoreCase))
        {
    ...
    {
        Prim->SetGenerateOverlapEvents(Params->GetBoolField(TEXT("generate_overlap_events")));
    }

    FString PawnResponse;
    if (Params->TryGetStringField(TEXT("pawn_response"), PawnResponse))
    {
        if (PawnResponse.Equals(TEXT("Overlap"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionResponseToChannel(ECC_Pawn, ECR_Overlap);
        }
        else if (PawnResponse.Equals(TEXT("Block"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionResponseToChannel(ECC_Pawn, ECR_Block);
        }
        else if (PawnResponse.Equals(TEXT("Ignore"), ESearchCase::IgnoreCase))
        {
            Prim->SetCollisionResponseToChannel(ECC_Pawn, ECR_Ignore);
        }
    }

    if (USphereComponent* Sphere = Cast<USphereComponent>(Prim))
    {
        if (Params->HasField(TEXT("sphere_radius")))
        {
            Sphere->SetSphereRadius(static_cast<float>(Params->GetNumberField(TEXT("sphere_radius"))));
        }
    }
    ...
    Result->SetStringField(TEXT("collision_profile"), Prim->GetCollisionProfileName().ToString());
    Result->SetNumberField(TEXT("collision_enabled"), static_cast<int32>(Prim->GetCollisionEnabled()));
    Result->SetBoolField(TEXT("generate_overlap_events"), Prim->GetGenerateOverlapEvents());
    return Result;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCompileBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Persist to disk so changes survive editor restart
